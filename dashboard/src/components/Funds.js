...
import { addFunds, withdrawFunds, getPnL } from "../hooks/api";
import FundWindow from "../components/windows/FundWindow";
// FIX: Removed local 'funds' state — reading exclusively from GeneralContext
// FIX: Removed FetchFunds import — context already holds funds data
...
const Funds = () => {
  ...
  // FIX: Only use funds and setFunds from GeneralContext — removed duplicate local state
  const { user, setUser, showAlert, funds, setFunds, refreshFunds } =
    useContext(GeneralContext);

  // FIX: Only call getPnL on mount; funds are already loaded via GeneralContext
  useEffect(() => {
    getPnL()
      .then((pnlRes) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // FIX: Use context refreshFunds instead of local re-fetch
      await refreshFunds();
      setModalOpen(false);
      setAmount("");
    } catch (err) {
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td
                      style={{
                        color:
                          (user?.unrealizedPL ?? 0) >= 0 ? "green" : "red",
                      }}
                    >
                  ...
                  <tr>
                    ...
                    <td>
                      <strong>
                        {(
                          (funds?.collateralLiquid ?? 0) +
                          (funds?.collateralEquity ?? 0)
                        ).toFixed(2)}
                      </strong>
                    </td>
                  </tr>
