...
const Summary = () => {
  const { user, holdings, positions, loading, updateUnrealisedPnLThrottled } =
    useContext(GeneralContext);

  // FIX: formatINR now uses ₹ symbol instead of $ to match the INR-themed UI
  const formatINR = (val) =>
  ...
  // FIX: Use throttled version (max once per 30s) instead of firing on every price tick
  useEffect(() => {
    if (holdings.length > 0 || positions.length > 0) {
      updateUnrealisedPnLThrottled(unrealised);
    }
  }, [unrealised, holdings.length, positions.length, updateUnrealisedPnLThrottled]);
