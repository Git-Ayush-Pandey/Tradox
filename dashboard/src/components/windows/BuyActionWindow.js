...
const BuyActionWindow = ({ uid, existingOrder = null }) => {
  ...
  // FIX: Read availableMargin directly from GeneralContext (already fetched)
  // instead of making a fresh FetchFunds() call on every modal open
  const { closeBuyWindow, showAlert, refreshFunds, refreshOrders, funds } =
    useContext(GeneralContext);

  const availableMargin = funds?.availableMargin ?? 0;
