...
// FIX: Single useContext(GeneralContext) call — was duplicated, one unused
...
const AnalyticsWindow = ({ stock, onClose }) => {
  ...
  // FIX: Single destructured call — removed the duplicate const { showAlert } line
  const { openBuyWindow, showAlert } = useContext(GeneralContext);
  ...
  useEffect(() => {
    const fetchPrice = async () => {
      ...
      try {
        ...
        const changeValue = +((1 - 1 / (1 + currPer / 100)) * currPrice).toFixed(2);
        setPriceData({ currPrice, currPer, changeValue });
      } catch (err) {
  ...
  const overviewData = overview
    ? [
        ...
        { label: "Debt/Equity", value: overview["totalDebt/totalEquityQuarterly"] },
        {
          label: "52W High",
