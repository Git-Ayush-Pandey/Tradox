...
export const enrichHoldingsandPositions = (item, price, basePrice) => {
  ...
  const safeRef = refPrice || item.avg || 1;
  const dayChange = (price - safeRef) * item.qty;
  const dayChangePercent = ((price - safeRef) / safeRef) * 100;
  const totalChange = price - item.avg;
  const totalChangePercent = item.avg ? (totalChange / item.avg) * 100 : 0;
...
export const GeneralContextProvider = ({ children }) => {
  ...
  // FIX: Throttle unrealised PnL API call — at most once per 30 seconds
  const lastPnlUpdateRef = useRef(0);
  const PNL_THROTTLE_MS = 30000;

  // FIX: Debounce executeMatchingOrders — waits 500ms after last livePrices tick
  const executeDebounceRef = useRef(null);
  ...
  // FIX: Debounced order matching — clears and resets 500ms timer on each livePrices update
  // instead of firing on every single tick (was firing dozens of times/second)
  useEffect(() => {
    ...
    executeDebounceRef.current = setTimeout(async () => {
        ...
        try {
          ...
          if (order.mode === "SELL") {
            ...
            if (order.type === "Delivery") {
              ...
              if (holding) buyPrice = holding.avg;
            } else {
              const position = positions.find((p) => p.name === order.name);
              if (position) buyPrice = position.avg;
            }
  ...
  // FIX: Throttled unrealised PnL update — at most once per 30 seconds
  const updateUnrealisedPnLThrottled = useCallback(
    (unrealised) => {
      const now = Date.now();
      if (now - lastPnlUpdateRef.current < PNL_THROTTLE_MS) return;
      lastPnlUpdateRef.current = now;
      updateUnrealisedPnL(unrealised).catch((err) =>
        console.error("Error updating unrealised PnL:", err)
      );
    },
    [] // eslint-disable-line
  );
