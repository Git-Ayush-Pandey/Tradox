...
// FIX: PRICE_API_BASE was undefined — now derived from PORT env var (same server)
const PRICE_API_BASE = process.env.PRICE_API_BASE ||
  `http://localhost:${process.env.PORT || 5000}/stock`;
...
async function autoSquareOffIntraday() {
  ...
  while (true) {
    // FIX: Removed { type: "Intraday" } filter — PositionsModel has no 'type' field.
    // All positions are intraday by nature (holdings = delivery).
    ...
    try {
      ...
      const orderDoc = {
        ...
        executed: true,
        executedAt: new Date(),
        placedAt: pos.boughtday || new Date(),
        auto: true,
