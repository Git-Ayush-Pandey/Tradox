...
router.get("/", isLoggedIn, async (req, res) => {
  try {
    ...
    all.forEach((item) => {
      ...
      // Only include non-sentinel items (items with a symbol)
      if (item.symbol) grouped[list].push(item);
    });

    // Ensure every list name appears in grouped even if empty
    // Find sentinel docs (no symbol) to discover empty lists
    const allListNames = [...new Set(all.map((i) => i.listName))];
...
// FIX: createWatchlist now saves a sentinel document to MongoDB so the list
// persists across restarts even when empty. Symbol is null for sentinel docs.
router.post("/create", isLoggedIn, async (req, res) => {
  ...
  try {
    ...
    // Save a sentinel document so the list is persisted to DB
